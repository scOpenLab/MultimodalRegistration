...
TO DO:
- Add support for Xenium Output V4

## Registration on the whole slide:

- Xenium-CODEX/slide_registration/
    - xenium_codex_slide_registration.py
    - registered_core_splitter.py

The registration is performed on the whole slide, eventually areas can be cropped with the `registered_core_splitter.py` script.
If the slide has a TMA or more than one tissue block, one tends to get better results with the TMA workflow.

### XeniumCodexRegistrator.py

This script aligns a CODEX image with a Xenium spatial transcriptomics dataset and saves the result as a [`SpatialData`](https://spatialdata.scverse.org/) object,
with the registered CODEX image and filtered Xenium transcript table.

To DO: The script needs the CODEX image and Xenium morphology focus image to be in zarr format.
It should be updated to work with the original tiffs.
...
The script is at:
- Xenium-CODEXTMA_core_registration/xenium_codex_core_registration.py

#### Usage

Usage of xenium_codex_core_registration.py:
